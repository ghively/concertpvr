...
def fake_probe():
    info = StreamInfo(
        youtube_id="phish-x",
        url="https://www.youtube.com/watch?v=phish-x",
        title="Phish — Dick's Night 2",
        channel_name="Phish",
        is_live=True,
        thumbnail_url=None,
    )
    ...
    with (
        patch("concertpvr.api.schedules.probe", side_effect=_async_probe) as m,
...
def test_post_schedule_with_url_creates_stream_and_schedule(client, fake_probe):
    _, info = fake_probe
    starts = dt.datetime(2099, 5, 1, 19, 0, tzinfo=dt.UTC)
    ends = dt.datetime(2099, 5, 1, 21, 0, tzinfo=dt.UTC)
    r = client.post(
        "/api/schedules",
        json={
            "url": info.url,
...
def test_post_schedule_rejects_when_ends_before_starts(client, fake_probe):
    _, info = fake_probe
    starts = dt.datetime(2099, 5, 1, 21, 0, tzinfo=dt.UTC)
    ends = dt.datetime(2099, 5, 1, 19, 0, tzinfo=dt.UTC)
    r = client.post(
        "/api/schedules",
        json={
            "url": info.url,
...
def test_patch_schedule_updates_artist_and_times(client, fake_probe):
    ...
    new_starts = dt.datetime(2099, 5, 1, 20, 0, tzinfo=dt.UTC)
    r = client.patch(
