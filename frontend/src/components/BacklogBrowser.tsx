...
import { useWatcherBacklog } from "@/lib/query";
...
export function BacklogBrowser({ watcherId }: { watcherId: number }) {
  ...
  const allItems = useMemo(() => {
    const current = data ?? [];
    // Flatten all prior pages and add current
    ...
    <div>
      ...
      {isError && (
        <p className="text-red-400 text-xs py-8 text-center">
          Failed to load backlog. The backend endpoint may not be implemented yet.
        </p>
      )}
